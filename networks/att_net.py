...
class UNet11(nn.Module):
    ...
    def forward(self, x, select="all"):
        ...
        if select == "all":
            ...
            final_2 = self.final_2(dec1)

            return [final_1,final_2], dec1
...
class Soft_Att(nn.Module):
    ...
    def forward(self, output1, output2, uncer_output,  feat):   # ori_pic 为feature
        fea = self.conv_block(feat) # [4, 1, 256, 256]
        # print(fea.size())
        soft_output_1 = self.Soft(output1)
        # print(soft_output_1.size())
        soft_output_2 = self.Soft(output2)
        # soft_uncer = self.Soft(uncer_output, fea) + fea
        final1 = torch.cat([soft_output_1,  uncer_output], dim=1) # [1, 4, 256, 256]
        final2 = torch.cat([soft_output_2,  uncer_output], dim=1) # [1, 4, 256, 256]
        print(final1.size())
        final1 = self.pred(final1)
        final2 = self.pred(final2)


        return [final1,final2], soft_output_1, fea
