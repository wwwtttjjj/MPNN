...
class UNet11(nn.Module):
    def __init__(self, num_filters=32, pretrained=False):
        ...
        #######################################################
        # self.dec25 = DecoderBlock(num_filters * (16 + 8), num_filters * 8 * 2, num_filters * 8)
        # self.dec24 = DecoderBlock(num_filters * (16 + 8), num_filters * 8 * 2, num_filters * 4)
        # self.dec23 = DecoderBlock(num_filters * (8 + 4), num_filters * 4 * 2, num_filters * 2)
        # self.dec22 = DecoderBlock(num_filters * (4 + 2), num_filters * 2 * 2, num_filters)
        # self.dec21 = ConvRelu(num_filters * (2 + 1), num_filters)
        self.final_1 = nn.Conv2d(num_filters, 1, kernel_size=1)
        self.final_2 = nn.Conv2d(num_filters, 1, kernel_size=1)
        # self.final_3 = nn.Conv2d(num_filters, 1, kernel_size=1)
    ...
    def forward(self, x):
        ...
        # dec25 = self.dec5(torch.cat([center, conv5], 1))
        # dec24 = self.dec4(torch.cat([dec25, conv4], 1))
        # dec23 = self.dec3(torch.cat([dec24, conv3], 1))
        # dec22 = self.dec2(torch.cat([dec23, conv2], 1))
        # dec21 = self.dec1(torch.cat([dec22, conv1], 1))

        dec5 = self.dec5(torch.cat([center, conv5], 1))
        dec4 = self.dec4(torch.cat([dec5, conv4], 1))
        dec3 = self.dec3(torch.cat([dec4, conv3], 1))
        dec2 = self.dec2(torch.cat([dec3, conv2], 1))
        dec22 = self.dec21(torch.cat([dec3, conv2], 1))
        ...
        final_2 = self.final_2(dec12)

        return [final_1,final_2], [dec2, dec22]
